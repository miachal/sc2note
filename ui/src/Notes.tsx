...
const Notes: React.FC<{ id: String }> = ({ id }) => {
  ...
  const [addNote] = useMutation(ADD_NOTE, {
    update(cache: any, { data: { addNote } }) {
      const { getNotes } = cache.readQuery({ query: GET_NOTES, variables: { id } });
      cache.writeQuery({
        query: GET_NOTES,
        variables: { id },
    ...
    <NotesContainer>
      <ListContainer>
        <List
          ...
          dataSource={[...data.getNotes].sort((a: { created: number; }, b: { created: number; }): any => a.created - b.created < 0 ? 1 : -1)}
          renderItem={(item: any) => (
            <List.Item
              actions={[
