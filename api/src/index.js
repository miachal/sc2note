...
const { searchPlayers } = require('./scraper/rankedftw');
const { getPlayerInformation } = require('./scraper/scrape');

const { diffInMinutes } = require('./helpers');
...
const resolvers = {
  Query: {
    searchProfilesByNames: async (p, { names }) => {
      const profiles = []; // [{id, name}]
      for (let j = 0; j < names.length; ++j) {
        const r = await searchPlayers(names[j]);
        if (r) profiles.push(r);
      }

      const ids = profiles.flat().reduce((t, c) => [...t, +c.id], []);
      const players = [];
      ...
      for (let j = 0; j < ids.length; ++j) {
        ...
        if (!player || (player && diffInMinutes(player.updatedAt) >= 60)) {
          console.log('No coz. Szukam na nowo.');
    ...
    searchProfile: async (p, { id }) => {
      let player = await Player.findOne({
        rankedftwId: id,
      });

      if (!player || diffInMinutes(player.updatedAt) >= 5) {
  ...
  Mutation: {
    addNote: async (p, { note: { players, icon, note } }) => {
